...
describe('parseShareDeepLink', () => {
  ...
  it('parses readest-remote://share/{token}', () => {
    expect(parseShareDeepLink(`readest-remote://share/${VALID_TOKEN}`)).toEqual({
      token: VALID_TOKEN,
    });
  });
  ...
  it('rejects tokens of the wrong length', () => {
    expect(parseShareDeepLink('readest-remote://share/short')).toBeNull();
    expect(parseShareDeepLink(`readest-remote://share/${VALID_TOKEN}extra`)).toBeNull();
  });
  ...
  it('rejects tokens with disallowed characters', () => {
    ...
    expect(parseShareDeepLink(`readest-remote://share/${bad}`)).toBeNull();
  });
  ...
  it('rejects other Remote hosts and the official readest scheme', () => {
    expect(parseShareDeepLink(`readest-remote://book/${VALID_TOKEN}`)).toBeNull();
    expect(parseShareDeepLink(`readest-remote://annotation/${VALID_TOKEN}`)).toBeNull();
    expect(parseShareDeepLink(`readest://share/${VALID_TOKEN}`)).toBeNull();
  });
